...
import type { ComponentRegistry, HydrationEntry, HydrationManifest } from '@docvia/renderer-core';
...
function mountIsland(
    ...
    if (!resolved) {
        console.error(`[docvia] Cannot hydrate: component "${entry.name}" not in registry`);
        return;
    }
    ...
    try {
        ...
        const element = React.createElement(Component, entry.props as Record<string, unknown>);
