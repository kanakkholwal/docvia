...
// Public API
export function transformToIR(ast, frontmatter, filePath) {
    const ctx = {
        headings: [],
        dependencies: [],
    ...
    const children = transformChildren(ast.children, filePath, ctx);
...
function transformChildren(nodes, filePath, ctx) {
    const result = [];
...
function transformNode(node, filePath, ctx) {
    ...
    switch (node.type) {
        case 'heading': {
            ...
            return {
                ...
                children: transformChildren(h.children, filePath, ctx),
            };
        }
        case 'paragraph':
            return {
                ...
                id: nodeId,
                props: {},
                children: transformChildren(node.children, filePath, ctx),
            };
        case 'text':
            return { type: 'text', id: nodeId, props: { value: node.value }, children: [] };
        case 'code': {
            const c = node;
            return {
                type: 'code-block',
        ...
        case 'link': {
            ...
            return {
                ...
                children: transformChildren(lnk.children, filePath, ctx),
            };
        }
        ...
        case 'leafDirective': {
            ...
            return {
                type: node.type === 'containerDirective' ? 'component' : 'component-inline',
                id: nodeId,
                props: {
                    name: d.name,
                    attributes,
                ...
                children: d.children ? transformChildren(d.children, filePath, ctx) : [],
            };
        }
        case 'emphasis':
            return {
        ...
        default: {
            ...
            return {
                ...
                children: 'children' in node
                    ? transformChildren(node.children || [], filePath, ctx)
                    : [],
            };
        }
    }
...
// Helpers
function addDependency(ctx, dep) {
    let normalized;
    ...
    const key = normalized.type === 'component' ? `c:${normalized.name}` : `${normalized.type}:${normalized.path}`;
...
function computeSlug(filePath, explicitSlug) {
    if (explicitSlug)
        return explicitSlug;
    return filePath
        .replace(/\\/g, '/')
