...
describe('resolvePlugins sorting', () => {
	...
	it('handles mixed phases and priorities', () => {
		const plugins: docviaPlugin[] = [
			...
			{ name: 'n-normal-100', version: '1.0.0', phase: 'normal', priority: 100 },
		];
		...
		expect(names.indexOf('n-normal-50')).toBeLessThan(names.indexOf('n-normal-100'));

		// All pre before normal
		expect(names.indexOf('p-pre-100')).toBeLessThan(names.indexOf('n-normal-50'));

		// All normal before post
		expect(names.indexOf('n-normal-100')).toBeLessThan(names.indexOf('p-post-200'));
	});
