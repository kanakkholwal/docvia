<script lang="ts">
    ...
    function isElement(node: any): node is Extract<RenderOutput, { kind: "element" }> {
        return node?.kind === "element";
    }
    function isText(node: any): node is Extract<RenderOutput, { kind: "text" }> {
        return node?.kind === "text";
    }
    function isComponent(node: any): node is Extract<RenderOutput, { kind: "component" }> {
        return node?.kind === "component";
    }
    function isFragment(node: any): node is Extract<RenderOutput, { kind: "fragment" }> {
