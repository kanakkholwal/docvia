...
import { JSONIFY_TYPE_DECL } from "@docvia/schema";
...
/**
 * The typed collection bindings + `docviaSource` aggregation shared verbatim by
 * the disk `source.ts` and `browser.ts` emitters. They differ only in how each
 * page's module is loaded (eager static imports vs lazy `() => import()`), which
 * is emitted above this tail; from `createCollection` onward the output is
 * identical, so it lives here once.
 */
function diskCollectionBindings(
...
function generateSourceTs(
	...
	parts.push(...diskCollectionBindings(collections));
...
function generateTypesTs(collections: readonly CollectionData[]): string {
	const parts: string[] = [
		"// Auto-generated by @docvia/compiler — do not edit manually",
		"",
		// `frontmatterTypeDef` is composed as `Jsonify<...>`, so the helper has to
		// be declared in the same file — the generated d.ts imports nothing.
		JSONIFY_TYPE_DECL,
	];
	for (const c of collections) {
