...
// Variable name sanitization for generated imports

function slugToVarName(collection: string, slug: string): string {
	return `_${collection}_${slug.replace(/[^a-zA-Z0-9]/g, "_")}`;
}
...
function generateBrowserTs(collections: readonly CollectionData[]): string {
	const parts: string[] = [
	...
	for (const c of collections) {
		...
		parts.push(
			"",
			`export const ${c.name}: Record<string, () => Promise<any>> = {`,
			entries,
			"};",
		);
	}
...
function generateServerTs(collections: readonly CollectionData[]): string {
	const parts: string[] = [
		"// Auto-generated by @docvia/compiler — do not edit manually",
	];

	for (const c of collections) {
		const slugs = Object.keys(c.routes);

		// Static imports
		for (const slug of slugs) {
			const varName = slugToVarName(c.name, slug);
			parts.push(
				`import * as ${varName} from ${JSON.stringify(c.routes[slug])};`,
			);
		}
		...
		const entries = slugs
			.map(
				(slug) =>
					`  ${JSON.stringify(slug)}: ${slugToVarName(c.name, slug)}`,
			)
			.join(",\n");

		parts.push(
			"",
			`export const ${c.name}: Record<string, any> = {`,
			entries,
			"};",
		);
	}
...
function generateDynamicTs(): string {
	return `// Auto-generated by @docvia/compiler — do not edit manually
...
export async function loadModule(
  ...
  if (typeof window === 'undefined') {
    ...
    return (server as any)[collection]?.[slug];
  }
  const browser = await import('./browser.js');
  const loader = (browser as any)[collection]?.[slug];
...
export async function getEagerModules(
  ...
  if (typeof window === 'undefined') {
    const server = await import('./server.js');
    return (server as any)[collection] ?? null;
  }
  return null;
}
`;
...
function generateSourceTs(
	...
	const parts: string[] = [
		...
		"import { loadModule, getEagerModules } from './dynamic.js';",
	];

	// Optional registry (if components configured)
	const hasRegistry =
	...
	if (hasRegistry) {
		...
		for (const [, entry] of entries) {
			// Resolve component path relative to outDir
	...
	for (const c of collections) {
		...
		parts.push(
			"",
			`const _eager_${c.name} = await getEagerModules(${JSON.stringify(c.name)});`,
			"",
...
export async function compile(
	...
	await Promise.all([
		writeFile(
			...
			generateBrowserTs(collectionData),
		),
		writeFile(
			join(resolvedOutDir, "server.ts"),
			generateServerTs(collectionData),
		),
		writeFile(join(resolvedOutDir, "dynamic.ts"), generateDynamicTs()),
		writeFile(
			join(resolvedOutDir, "source.ts"),
			generateSourceTs(collectionData, resolvedOutDir, config),
		),
		writeFile(
