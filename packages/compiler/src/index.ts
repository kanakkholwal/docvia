...
function generateRegistryTs(
	...
	return [
		...
		mapEntries,
		"    };",
		"    return (map as Record<string, unknown>)[name] ?? null;",
		"  },",
		"};",
		"",
	].join("\n");
...
export async function compile(
	...
	for (const collection of collections) {
		...
		await compileParallel(files, async (file) => {
			...
			routes[slug] =
				`/${relative(process.cwd(), file.path).replace(/\\/g, "/")}?docvia`;
		...
		await Promise.all([
			writeFile(
				join(collectionOutDir, "meta.json"),
				JSON.stringify(pages, null, 2),
			),
			writeFile(
				join(collectionOutDir, "nav.json"),
				JSON.stringify(nav, null, 2),
			),
			writeFile(
				join(collectionOutDir, "tags.json"),
	...
	if (config.components && Object.keys(config.components).length > 0) {
		...
		await writeFile(
			...
			generateRegistryTs(
				normalizedComponents as Record<
					string,
					{
						path: string;
						hydrate?: boolean;
