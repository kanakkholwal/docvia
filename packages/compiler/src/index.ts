...
function generateDynamicTs(
	...
	// Embed the route→filepath map so dynamic.ts can call loadMarkdown for Node SSR
	const routeMaps = collections
		.map((c) => {
			const entries = Object.entries(c.routes)
				.map(
					([slug, routePath]) =>
						`    ${JSON.stringify(slug)}: ${JSON.stringify(routePath.replace(/\?docvia$/, ""))}`,
				)
				.join(",\n");
...
export async function loadModule(
  ...
  if (typeof window === 'undefined') {
    // Node / Next.js SSR: use loadMarkdown to compile on-the-fly
    const filePath = routeMap[collection]?.[slug];
    if (!filePath) return undefined;
    const { fileURLToPath } = await import('node:url');
    const resolved = fileURLToPath(new URL(filePath, import.meta.url));
...
export async function getEagerModules(
  ...
  if (typeof window === 'undefined') {
    // Node / Next.js: load all pages via loadMarkdown
