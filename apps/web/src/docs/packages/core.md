...
`@docvia/core` is the markdown parsing layer of docvia. It wraps the [`unified`](https://unifiedjs.com/) ecosystem (`remark` for markdown, `rehype` for HTML) into a single function, `parseMarkdown`, that takes a raw markdown string and returns a sanitized HAST (HTML AST) tree ready to be handed to `transformToIR` from `@docvia/ir`.
