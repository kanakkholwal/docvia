...
> The `id` field on `element` is optional, but on `component` it is **required**, because every component island must have a stable id so the hydration manifest can locate its `[data-hid]` anchor in the DOM.
